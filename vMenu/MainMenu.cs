...
namespace vMenuClient
{
    public class MainMenu : BaseScript
    {
        ...
        public static ReportsMenu ReportMenu { get; private set; }
        ...
        {
            get { return NoClip.IsNoclipActive(); }
        ...
        public static bool EnableExperimentalFeatures =
            (GetResourceMetadata(GetCurrentResourceName(), "experimental_features_enabled", 0) ?? "0") == "1";

        public static string Version { get { return GetResourceMetadata(GetCurrentResourceName(), "version", 0); } }

        public static bool DontOpenMenus
        {
            get { return MenuController.DontOpenAnyMenu; }
            set { MenuController.DontOpenAnyMenu = value; }
        }

        public static bool DisableControls
        {
            get { return MenuController.DisableMenuButtons; }
...
#pragma warning disable CS8793 // The given expression always matches the provided pattern.
                    ...
                    {
                        if (!kvp.StartsWith("settings_") && !kvp.StartsWith("vmenu") && !kvp.StartsWith("veh_") &&
                            !kvp.StartsWith("ped_") && !kvp.StartsWith("mp_ped_"))
                        {
...
#pragma warning restore CS8793 // The given expression always matches the provided pattern.
            ...
            string KeyMappingID = String.IsNullOrWhiteSpace(GetSettingsString(Setting.vmenu_keymapping_id))
                ? "Default"
                : GetSettingsString(Setting.vmenu_keymapping_id);
            RegisterCommand($"vMenu:{KeyMappingID}:NoClip", new Action<dynamic, List<dynamic>, string>(
                (dynamic source, List<dynamic> args, string rawCommand) =>
                {
                    if (IsAllowed(Permission.NoClip))
                    {
                        if (Game.PlayerPed.IsInVehicle())
                        {
                            ...
                            else
                            {
                                NoClipEnabled = false;
                                Notify.Error(
                                    "This vehicle does not exist (somehow) or you need to be the driver of this vehicle to enable noclip!");
                            }
                        }
                        else
                        {
            ...
            RegisterCommand($"vMenu:{KeyMappingID}:MenuToggle", new Action<dynamic, List<dynamic>, string>(
                (dynamic source, List<dynamic> args, string rawCommand) =>
                {
                    if (MenuEnabled)
                    {
            ...
            RegisterKeyMapping($"vMenu:{KeyMappingID}:MenuToggle", "vMenu Toggle Button Controller",
                "pad_digitalbuttonany", "start_index");

            #endregion

            if (EnableExperimentalFeatures)
            {
                RegisterCommand("testped", new Action<dynamic, List<dynamic>, string>(
                    (dynamic source, List<dynamic> args, string rawCommand) =>
                    {
                ...
                RegisterCommand("tattoo", new Action<dynamic, List<dynamic>, string>(
                    (dynamic source, List<dynamic> args, string rawCommand) =>
                    {
                        if (args != null && args[0] != null && args[1] != null)
                        {
                            Debug.WriteLine(args[0].ToString() + " " + args[1].ToString());
                            TattooCollectionData d = Game.GetTattooCollectionData(int.Parse(args[0].ToString()),
                ...
                RegisterCommand("clearfocus", new Action<dynamic, List<dynamic>, string>(
                    (dynamic source, List<dynamic> args, string rawCommand) =>
                    {
            ...
            RegisterCommand("vmenuclient", new Action<dynamic, List<dynamic>, string>(
                (dynamic source, List<dynamic> args, string rawCommand) =>
                {
                    if (args != null)
                    {
                        if (args.Count > 0)
                        {
                            ...
                            {
                                Notify.Info(
                                    "A full config dump will be made to the console. Check the log file. This can cause lag!");
                                Debug.WriteLine("\n\n\n########################### vMenu ###########################");
                                Debug.WriteLine(
                                    $"Running vMenu Version: {Version}, Experimental features: {EnableExperimentalFeatures}, Debug mode: {DebugMode}.");
            ...
            if (GetCurrentResourceName() != "vMenu")
            {
                ...
                throw new Exception(
                    ...
                    GetCurrentResourceName() + "' to 'vMenu' (case sensitive)!\n");
            }
            else
            {
                Tick += OnTick;
        ...
        private static void PostPermissionsSetup()
        {
            ...
            {
                // Manage Stamina
                if (PlayerOptionsMenu != null && PlayerOptionsMenu.PlayerStamina &&
        ...
        private async Task OnTick()
        {
            ...
            if (ConfigOptionsSetupComplete)
            {
                ...
                if (Game.IsDisabledControlJustReleased(0, Control.PhoneCancel) && MpPedCustomization.DisableBackButton)
                {
                    await Delay(0);
                    Notify.Alert(
                        "You must save your ped first before exiting, or click the ~r~Exit Without Saving~s~ button.");
                }
        ...
        private static void CreateSubmenus()
        {
            ...
            if (IsAllowed(Permission.OPMenu))
            {
                ...
                var button = new MenuItem("Online Players", "All currently connected players.") { Label = "→→→" };
            ...
            if (IsAllowed(Permission.OPUnban) || IsAllowed(Permission.OPViewBannedPlayers)||
                IsAllowed(Permission.POViewReports))
            {
                var moderationToolsSubmenu = new Menu("vMenu",
                    "Moderation Tools-/Actions");
                var moderatoinToolsBtn = new MenuItem("Moderation Tools-/Actions") {Label = "→→→"};
                MenuController.AddSubmenu(Menu, moderationToolsSubmenu);
                MenuController.BindMenuItem(Menu, moderationToolsSubmenu, moderatoinToolsBtn);
                Menu.AddMenuItem(moderatoinToolsBtn);

                BannedPlayersMenu = new BannedPlayers();
                var menu = BannedPlayersMenu.GetMenu();
                var button = new MenuItem("Banned Players",
                    "Manage Banned Players here.") { Label = "→→→" };
                AddMenu(moderationToolsSubmenu, menu, button);

                ReportMenu = new ReportsMenu();
                var reportMenu = ReportMenu.GetMenu();
                var reportButton = new MenuItem("View Reports",
                    "Shows all open reports. If the owner has double-claims off, claimed reports will be locked.") { Label = "→→→" };
                AddMenu(moderationToolsSubmenu, reportMenu, reportButton);
            ...
            var playerSubmenuBtn =
                new MenuItem("Player Related Options", "Open this submenu for player related subcategories.")
                {
                    Label = "→→→"
                };
            ...
            if (IsAllowed(Permission.POMenu))
            {
                ...
                var button =
                    new MenuItem("Player Options", "Common player options can be accessed here.") { Label = "→→→" };
                AddMenu(PlayerSubmenu, menu, button);
            }

            var vehicleSubmenuBtn =
                new MenuItem("Vehicle Related Options", "Open this submenu for vehicle related subcategories.")
                {
                    Label = "→→→"
                };
            ...
            if (IsAllowed(Permission.VOMenu))
            {
                ...
                var button = new MenuItem("Vehicle Options",
            ...
            if (IsAllowed(Permission.VSMenu))
            {
                ...
                var button =
                    new MenuItem("Vehicle Spawner", "Spawn a vehicle by name or choose one from a specific category.")
                    {
                        Label = "→→→"
                    };
                AddMenu(VehicleSubmenu, menu, button);
            ...
            if (IsAllowed(Permission.SVMenu))
            {
                ...
                var button =
                    new MenuItem("Saved Vehicles", "Save new vehicles, or spawn or delete already saved vehicles.")
                    {
                        Label = "→→→"
                    };
                AddMenu(VehicleSubmenu, menu, button);
            ...
            if (IsAllowed(Permission.PVMenu))
            {
                ...
                var button = new MenuItem("Personal Vehicle",
            ...
            if (IsAllowed(Permission.PAMenu))
            {
                ...
                var button = new MenuItem("Player Appearance",
                    "Choose a ped model, customize it and save & load your customized characters.") { Label = "→→→" };
                AddMenu(PlayerSubmenu, menu, button);

                MpPedCustomizationMenu = new MpPedCustomization();
                var menu2 = MpPedCustomizationMenu.GetMenu();
                var button2 = new MenuItem("MP Ped Customization",
            ...
            var worldSubmenuBtn =
                new MenuItem("World Related Options", "Open this submenu for world related subcategories.")
                {
                    Label = "→→→"
                };
            ...
            if (IsAllowed(Permission.TOMenu) && GetSettingsBool(Setting.vmenu_enable_time_sync))
            {
                ...
                var button =
                    new MenuItem("Time Options", "Change the time, and edit other time related options.")
                    {
                        Label = "→→→"
                    };
            ...
            if (IsAllowed(Permission.WOMenu) && GetSettingsBool(Setting.vmenu_enable_weather_sync))
            {
                ...
                var button =
                    new MenuItem("Weather Options", "Change all weather related options here.") { Label = "→→→" };
                AddMenu(WorldSubmenu, menu, button);
            }
            ...
            if (IsAllowed(Permission.WPMenu))
            {
                ...
                var button =
                    new MenuItem("Weapon Options", "Add/remove weapons, modify weapons and set ammo options.")
                    {
                        Label = "→→→"
                    };
                AddMenu(PlayerSubmenu, menu, button);
            ...
            if (IsAllowed(Permission.WLMenu))
            {
                ...
                var button =
                    new MenuItem("Weapon Loadouts", "Mange, and spawn saved weapon loadouts.") { Label = "→→→" };
                AddMenu(PlayerSubmenu, menu, button);
            }
            ...
            if (IsAllowed(Permission.VCMenu))
            {
                ...
                var button = new MenuItem("Voice Chat Settings", "Change Voice Chat options here.") { Label = "→→→" };
                AddMenu(Menu, menu, button);
            }

            {
                ...
                var button = new MenuItem("Recording Options", "In-game recording options.") { Label = "→→→" };
                AddMenu(Menu, menu, button);
            }
            ...
            // Add misc settings menu.
            {
                ...
                var button = new MenuItem("Misc Settings",
            ...
            var btn = new MenuItem("About vMenu", "Information about vMenu.") { Label = "→→→" };
